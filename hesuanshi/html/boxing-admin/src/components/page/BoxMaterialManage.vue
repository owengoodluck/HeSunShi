    ...
    <div>
        ...
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><i class="el-icon-menu"></i> 材质列表管理</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <el-col :span="24" class="warp-main">
            ...
            <el-table :data="tableData" border ref="multipleTable" highlight-current-row stripe>
                ...
                <el-table-column prop="grams" label="克数"></el-table-column>
        ...
        <!--新增材质-->
        <el-dialog width="960px" title="新增材质" :visible.sync ="addFormVisible" :close-on-click-modal="false">
            <el-form :model="addForm" status-icon label-width="120px" :rules="addFormRules" ref="addForm">
                <el-row >
                    <el-col :span="12">
                        <el-form-item label="选择材质分类" prop="classifyId">
                ...
                <el-row >
                    ...
                    <el-col :span="8">
                        <el-form-item label="克数" prop="grams">
                            <el-input type="number" v-model.trim="addForm.grams" placeholder="单位:克">></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
        ...
        <!--编辑界面-->
        <el-dialog width="960px" title="编辑材质" :visible.sync ="editFormVisible" :close-on-click-modal="false">
            <el-form :model="editForm" status-icon label-width="120px" :rules="editFormRules" ref="editForm">
               <el-row >
                ...
                <el-row >
                    ...
                    <el-col :span="8">
                        <el-form-item label="重量" prop="grams">
                            <el-input type="number" v-model.trim="editForm.grams" placeholder="单位:克"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
...
<script>
    export default {
        data() {
            ...
            return {
                ...
                addFormRules: {
                    ...
                    grams: [
                        {required: true, message: '请输入重量克数', trigger: 'blur'},
                        {validator: validateNormalprice, trigger: 'blur'}
                    ],
                    image: [
                        {required: true, message: '请输上传材质图片', trigger: 'blur'}
                    ]
                },
                addForm: {
                    classifyId: '',
        ...
        mounted(){
            ...
            this.$axios.all([this.$axios.get('boxMaterialClassify/findClassify'),this.$axios.get('boxType/findBoxType'),this.$axios.get(that.url + params)])
        ...
        methods: {
            ...
            addSubmit: function () {
                ...
                this.$refs.addForm.validate((valid) => {
                    if (valid) {
                        ...
                        that.$axios.post('boxMaterial/insert', params)
            ...
            editSubmit: function () {
                ...
                this.$refs.editForm.validate((valid) => {
                    if (valid) {
                        ...
                        that.$axios.post('boxMaterial/update', params)
                        .then(function (response) {
                            if (response.data.code == 200){
            ...
            updateStatus:function(row){
                ...
                that.$axios.post('boxMaterial/updateStatus', params)
                .then(function (response) {
                    if (response.data.code == 200){
            ...
            deleteSubmit: function (index, row) {
                ...
                this.$confirm('确认删除该记录吗?', '提示', {type: 'warning'}).then(() => {
                    ...
                    that.search();
                    that.$axios.post('boxMaterial/delete/' + row.id)
                    .then(function (response) {
                        if (response.data.code == 200){
