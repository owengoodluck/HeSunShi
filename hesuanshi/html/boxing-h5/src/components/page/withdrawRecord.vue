		...
		<scroller style="top: 44px"
			...
			<div v-for="(item) in recordList">
				<div class="listTable">
					...
					<div class="status" v-if="item.status == 0" style="color: #ff982e">待处理</div>
					<div class="status" v-else-if="item.status == 1" style="color: #fb6363">处理中</div>
					<div class="status" v-else-if="item.status == 2" style="color: #34c6be">已处理</div>
    ...
    export default {
        ...
        methods: {
        	findTixianRecord () {
			    ...
			    that.$axios.post('merchantOrder/findTixianRecord', params)
